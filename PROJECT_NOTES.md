...
- The figure is one skinned mesh generated in code (`lib/bodyMesh.ts`) over a Mixamo-named skeleton, posed by `lib/skinnedFigure.ts` from the same poses. Body shape changes go in the generator's ring radii and bulges, never in the poses; the face, hair, sneakers and fists are still the mannequin's, placed from the pose. Clothes are shells a few millimetres off an all-skin body (open hems with a lip, necklines cut by a per-vertex mask that the shadow pass shares); slack is charged against pads by the clearance sweep, so keep hems within a centimetre of the skin. A downloaded rigged GLB can replace the body through the viewer's `figure: { url }` option for experiments.
