...
- `buildProgram()` filters catalog exercises by their `difficulty` field against `maxDifficultyRank`. This is a **strong preference, not a hard filter**: if a keyword only returned exercises above the user's level, a too-hard exercise still beats an empty slot, since enough empty slots sink the whole program back to the built-in fallback roster. Injury-safety and media availability remain hard requirements.
