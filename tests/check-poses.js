...
for (const [name, pose] of Object.entries(exercisePoses)) {
  ...
  frames.forEach((f, i) => {
    ...
    if (floor) {
      ...
      if (-deepest > 0.16 && name !== "legExtension" && name !== "dip" && name !== "stepUp" && name !== "boxJump" && name !== "broadJump" && name !== "tuckJump") note(`${where}: floats ${(-deepest).toFixed(3)} above the floor`);
    }
  });
...
const STANDING_FRAMES = {
  ...
  tricepsExtension: 0, jump: 0, boxJump: 0, broadJump: 0, tuckJump: 0, lateralLunge: 0, clean: 4, powerClean: 4, kettlebellSnatch: 3,
