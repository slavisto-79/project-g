...
for (const [name, pose] of Object.entries(exercisePoses)) {
  ...
  const floor = { calfRaise: 0.075, seatedCalfRaise: 0.04, bearCrawl: 0.09, gluteBridge: 0.1, singleLegGluteBridge: 0.1, frogPump: 0.075, benchDip: 0.1 }[name] ?? 0.12;
  ...
  // A forearm plank is genuinely longer and lower than anything else here. A
  ...
  // 3.8.
  const widthCap = name === "plank" ? 5.0 : name === "proneRaise" ? 4.5 : name === "hollowHold" || name === "bicycleCrunch" ? 4.0 : 3.4;
...
const STANDING_FRAMES = {
  ...
  tricepsExtension: 0, boxJump: 0, broadJump: 0, tuckJump: 0, jumpSquat: 0, depthJump: 0, lateralLunge: 0, clean: 0, powerClean: 4, kettlebellSnatch: 3, kettlebellClean: 2,
...
const RIGID_LEGS = {
  ...
  pikePushUp: null, plank: null, sidePlank: null, handstandPushUp: null,
...
const GAITS = new Set(["carry", "overheadCarry", "frontRackCarry", "suitcaseCarry", "run", "jumpingLunge", "skaterBound", "bearCrawl", "highKnees", "depthJump"]); // walking strides are asymmetric on purpose
for (const [name, pose] of Object.entries(exercisePoses)) {
...
// Every library exercise must demo as one of the authored movements. The
// library is the only source of exercises (the catalog is gone), so a name
// missing from POSE_FOR_EXERCISE would be a card with no figure, and a name
// pointing at a movement that does not exist would crash the card. Checked
// against the map actually in App.tsx.
{
  ...
  if (mapStart < 0) {
    note("library demos: cannot find POSE_FOR_EXERCISE in App.tsx");
  } else {
    const map = new Function(src.slice(mapStart, mapEnd).replace(": Record<string, PoseName>", "") + "\nreturn POSE_FOR_EXERCISE;")();
    const lib = mod("lib/exerciseLibrary.ts");
    for (const exercise of lib.exerciseLibrary) {
      const movement = map[exercise.name];
      if (!movement) note("library demos: " + exercise.name + " has no movement in POSE_FOR_EXERCISE");
      else if (!exercisePoses[movement]) note("library demos: " + exercise.name + " -> unknown movement " + movement);
    }
    for (const name of Object.keys(map)) {
      if (!lib.exerciseLibrary.some((exercise) => exercise.name === name)) note("library demos: POSE_FOR_EXERCISE names \"" + name + "\", which is not in the library");
    }
    console.log(lib.exerciseLibrary.length + " library exercises, every one with a demo");
  }
}
