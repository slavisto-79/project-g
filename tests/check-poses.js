...
for (const [name, pose] of Object.entries(exercisePoses)) {
  ...
  frames.forEach((f, i) => {
    ...
    if (floor) {
      ...
      if (-deepest > 0.16 && name !== "legExtension" && name !== "dip" && name !== "stepUp" && name !== "boxJump" && name !== "broadJump" && name !== "tuckJump" && name !== "jumpSquat") note(`${where}: floats ${(-deepest).toFixed(3)} above the floor`);
    }
  });
...
const STANDING_FRAMES = {
  ...
  overheadPress: 0, pushPress: 0, pushJerk: 0, lateralRaise: 0,
  tricepsExtension: 0, jump: 0, boxJump: 0, broadJump: 0, tuckJump: 0, jumpSquat: 0, lateralLunge: 0, clean: 4, powerClean: 4, kettlebellSnatch: 3, kettlebellClean: 2,
...
const GAITS = new Set(["carry", "overheadCarry", "frontRackCarry", "suitcaseCarry", "run", "jumpingLunge", "skaterBound", "bearCrawl", "highKnees"]); // walking strides are asymmetric on purpose
for (const [name, pose] of Object.entries(exercisePoses)) {
